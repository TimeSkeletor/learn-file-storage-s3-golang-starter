...
func (cfg apiConfig) getVideoAspectRatio(filePath string) (string, error) {
	storedCmd := strings.Fields(cfg.ffProbeCmd)
	storedCmd = append(storedCmd, filePath)

	cmd := exec.Command(storedCmd[0], storedCmd[1:]...)
...
func getAspectRatio (aspectRatio string) string {
	switch aspectRatio {
	case "16:9":
		return "landscape"
	case "9:16":
